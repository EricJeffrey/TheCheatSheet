...
//     using HandlerType = std::function<HandlerResult(const NlohmannJson &, const Headers&)>;

//     NlohmannJson mBody;
//     HandlerType mHandler;

//     BodyController(const string &path, const vector<string> &headerNames, HandlerType handler)
//         : BasicController(path, {}, headerNames), mHandler(handler) {}
//     ~BodyController() = default;

//     virtual HandlerResult invoke() override { return mHandler(mBody, mHeaders); }
